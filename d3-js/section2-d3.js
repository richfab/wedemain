(function() {
    ...
    $(window).bind("scroll", function(event) {
        ...
        if(!mapHasBeenShowed){
            ...
            $("#section2-map-limit:in-viewport").each(function() {
                mapHasBeenShowed = true;
                mapInit();
                $("#section2-websiteLi1").mouseover();
            });
    ...
    function mapInit(){
        ...
        $("#section2-map").height(mapHeight);
        $("#section2-map").width(mapHeight*2);
        
        var map = new Datamap({
            
            element: document.getElementById('section2-map'),
        ...
        $("#section2-websiteUl li").each(function(){
            $(this).mouseover(
                    function() {
                        changeWebsiteMap($(this).attr('data-url'),$(this).attr('data-index'));
                
                //le nom du site est mis en valeur
                $("#section2-websiteUl span").removeClass('selected');
                $(this).children("span").addClass('selected');
                
                //les donnees sont mises a jour
                $("#section2-distance").text(dataNumberToNice($(this).attr('distance')));
                $("#section2-terre-lune").text(dataNumberToNice($(this).attr('distance')/circonferenceTerre));
            });
        });
